...
class RadarVisionHybrid(object):
    ...
    def load_radar_objects(self, img_index):
        radar_objects = []
        if self.radar_data !=[]:
            if img_index >= len(self.radar_data):
                return []
            if len(self.radar_data[img_index]) == 0:
                return []
            data_dict = self.radar_data[img_index]
            data = []
        ...
        if self.corner_radar_data !=[]:
            if img_index >= len(self.corner_radar_data):
                return []
            if len(self.corner_radar_data[img_index]) == 0:
                return []
            data_dict = self.corner_radar_data[img_index]
            data = []
            for value in data_dict.values():
                data.extend(value)
            for data_e in data:
                ...
                vx = data_e.get('vel_lon', 999)
                vy = data_e.get('vel_lat', 999)
    ...
    def load_vision_objects(self, img_index):
        data = []
        vision_objects = []
        if len(self.vision_data[img_index]) == 0:
            return []
        if self.vision_keyword in ['_arcsoft_obj.txt'] or self.vision_keyword == '.txt':
            data_dict = self.vision_data[img_index] if self.mode else {'objAttributes': self.vision_data[img_index]["Oem"]["visInfo"][0]["obj"]["objAttributes"]}
            # data_dict = self.vision_data[img_index] if self.mode else {'objAttributes': self.vision_data[img_index]["perception"]["objects"]}  # pc
            for value in data_dict.values():
                data.extend(value)
            # Signal_img_index_data = self.Signal_data[img_index][str(img_index)]
            ...
            data_dict = self.vision_data[img_index]
            for value in data_dict.values():
                data.extend(value)
            for data_e in data:
    ...
    def load_J3_objects(self, img_index):
        J3_objects = []
        if self.J3_data != []:
            data_dict = self.J3_data[img_index]
            data = []
            for value in data_dict.values():
                data.extend(value)
    ...
    def load_reVeh_objects(self, img_index):
        data = []
        vision_objects = []
        if len(self.arc_reVeh_data[img_index]) == 0:
            return []
        data_dict = self.arc_reVeh_data[img_index]
        for value in data_dict.values():
            data.extend(value)
        for data_e in data:
            ObjClass = data_e.get('u8OemObjClass')
...
def make_parser():
    ...
    parser.add_argument(
        ...
        "--回灌后的arc算法结果",
        type=str,
        # default=r'\\hz-iotfs02\Model_Test\TestSpace\Personal_Space\CHZ\Southlake\adas_perception_v3.1_SPC030_2m_98\output\shaoyuqi\V3.1_2M_3.1.27223.1398\20251226',  # 素管素材结果
        default=r'F:\Desktop\python_tools\专项\专项KPI\txt',
    ...
    parser.add_argument(
        ...
        type=str,
        # default=r'',
        default=r"\\hz-iotfs02\Model_Test\TestSpace\Personal_Space\YJ\bugsacn\result\专项\20251230\all_VideoFilePath.txt",
