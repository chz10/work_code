...
def angle_diff(gt, dt):
    diff = gt - dt
    diff = (diff + 180) % 360 - 180  # 映射到 [-180, 180]
    return diff
...
def checker(pseudo_true_value_path,recharge_res_path,checker_save_path,checker_type):
    ...
    for pseudo_true_value in pseudo_true_values:
        ...
        pseudo_frame_type[frame] = pseudo_true_value[pseudo_attribute]
    ...
    for matched_group in matched_groups:
        ...
        for item in dt_obj_list:
            ...
            recharge_frame_type[list(item.keys())[0]] = math.degrees(item[list(item.keys())[0]]['f32Heading'])

    interval_frame = len(pseudo_frame_type)-1
    pseudo_no_type_recharge = 0
    pseudo_no_type_recharge_list = []
    for num,frame_num in enumerate(range(start_end_frame[0],start_end_frame[1]+1)):
        if abs(angle_diff(pseudo_frame_type.get(frame_num), recharge_frame_type.get(frame_num))) > heading_threshold:
            pseudo_no_type_recharge +=1
            pseudo_no_type_recharge_list.append(frame_num)
    pseudo_no_type_recharge_rate = round((interval_frame-pseudo_no_type_recharge)/interval_frame,2)
    # 先按帧号排序（因为字典可能无序）
    recharge_frame_type_frames = sorted(recharge_frame_type.items())

    recharge_frame_type_jump_count = 0
    recharge_frame_type_list = []
    for i in range(1, len(recharge_frame_type_frames)):
        prev_frame, prev_type = recharge_frame_type_frames[i - 1]
        curr_frame, curr_type = recharge_frame_type_frames[i]
        if abs(angle_diff(prev_type, curr_type)) > heading_threshold:
        # if prev_type != curr_type:  # 类型跳变
            recharge_frame_type_jump_count += 1
            recharge_frame_type_list.append((prev_frame,curr_frame))
    interval_frame = len(recharge_frame_type_frames)
    recharge_frame_type_rate = round((interval_frame-recharge_frame_type_jump_count)/interval_frame,2)

    save_file_path = os.path.join(checker_save_path,f'{os.path.basename(pseudo_true_value_path)[:-4]}_{checker_type}.json')
    is_checked = True
    attr = {
        'result': min(pseudo_no_type_recharge_rate, recharge_frame_type_rate) >= 1,
        'is_checked': is_checked,
        'rate': pseudo_no_type_recharge_rate if pseudo_no_type_recharge_rate < recharge_frame_type_rate else recharge_frame_type_rate,
        'other': {
            'clip_name': os.path.basename(pseudo_true_value_path)[:-4],
            'checker_type': checker_type,
            "start_end_frame": start_end_frame,
            'gt_id': dominant_id_list,
            '回灌版本与伪真值做比朝向通过率': pseudo_no_type_recharge_rate,
            '回灌版本与伪真值做比朝向不同次数': pseudo_no_type_recharge,
            '回灌版本与伪真值做比朝向不同帧数': pseudo_no_type_recharge_list,
            '回灌版本朝向通过率': recharge_frame_type_rate,
            '回灌版本朝向跳变次数': recharge_frame_type_jump_count,
            '回灌版本朝向跳变帧数': recharge_frame_type_list,
...
if __name__ == '__main__':
    ...
    try:
        ...
        os.makedirs(checker_save_path, exist_ok=True)  # 确保目录存在
        with open(save_file_path, 'w', encoding='utf-8') as f:
            json.dump(attr, f, ensure_ascii=False, indent=4)
