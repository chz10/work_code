...
def checker(pseudo_true_value_path,recharge_res_path,checker_save_path,checker_type):
    ...
    for matched_group in matched_groups:
        ...
        for item in dt_obj_list:
            ...
            recharge_frame_type[list(item.keys())[0]] = item[list(item.keys())[0]]['u8OemObjClass']
    ...
    for num,frame_num in enumerate(range(start_end_frame[0],start_end_frame[1]+1)):
        if pseudo_frame_type.get(frame_num) != recharge_frame_type.get(frame_num):
    ...
    attr = {
        ...
        'other': {
            ...
            '回灌版本与伪真值做比类型通过率': pseudo_no_type_recharge_rate,
            '回灌版本与伪真值做比类型不同次数': pseudo_no_type_recharge,
            '回灌版本与伪真值做比类型不同帧数': pseudo_no_type_recharge_list,
            '回灌版本类型通过率': recharge_frame_type_rate,
            '回灌版本类型跳变次数': recharge_frame_type_jump_count,
            '回灌版本类型跳变帧数': recharge_frame_type_list,
