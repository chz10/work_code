...
def checker(pseudo_true_value_path,recharge_res_path,checker_save_path,checker_type):
    ...
    pass_rate = match_pass_sum / total if total > 0 else 0
    ...
    attr = {
        ...
        'other': {
            ...
            "pass_id": df.loc[df['match_pass'], 'u8Id'].tolist() if pass_rate > 0 else [],
            "pass_frame": df.loc[df['match_pass'], 'frame_id'].tolist() if pass_rate > 0 else [],
